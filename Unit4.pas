...
begin
  ...
  students.Cells[0,0]:='№';
  students.Cells[1,0]:='Прізвище та ініціали студента';
...
begin
  ...
  if Arow = students.RowCount-1 then
    begin
      ...
      students.Cells[1,students.RowCount-1]:='';
...
var
  i:integer;
begin
  if sRow = students.RowCount-1 then
    exit;     
  students.RowCount:=students.RowCount+1;
...
begin
  ...
  if (sRow = students.RowCount-1) and (students.RowCount > 3) then
  begin
    students.Cells[1,sRow]:='';
    students.RowCount:=students.RowCount-1;
...
begin
  ...
  for i:=1 to students.RowCount-1 do
    if Length(students.Cells[1,i])>0 then
    begin
      Form1.StringGrid1.Cells[1,j]:=students.Cells[1,i];
